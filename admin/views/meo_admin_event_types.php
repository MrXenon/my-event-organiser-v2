...
<h4 class="text-uppercase">Event Types</h4>
    ...
    echo (($action == 'update') ? '<form action="' . $base_url . '" method="post">' : '');
        ?>
            <div class="col-md-8">
                    <?php
                    // als het type aantal kleiner is dan 1, toon ons een warning.
                    if ($builder->getNrOfEventTypes() < 1) {
                    ?>
                        <p class='alert alert-warning text-center'>No event type has been added yet!</p>
                    <?php } else { 
                        if($action !== 'update') {
                        ?>
                        ...
                        // koppel de type lijst.
                        $type = $builder->getEventTypeList();

                        // voor elk type, maken wij een nieuw object en laaden wij dit in in onze tabel.
                        foreach ($type as $builder_obj) {
